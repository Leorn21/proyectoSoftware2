...
describe('SearchBar', () => {
  test('REQ-F05 limpia la búsqueda y notifica el valor vacío', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Buscar inventario'), 'martillo');
...
describe('StockMovementList', () => {
  test('REQ-F03 muestra estado vacío de movimientos', () => {
    render(<StockMovementList movements={[]} onDelete={vi.fn()} productUnit="piezas" />);
...
describe('BatchList', () => {
  test('REQ-F02 muestra estado vacío de lotes', () => {
    render(<BatchList batches={[]} onEdit={vi.fn()} onDelete={vi.fn()} productUnit="piezas" />);
...
describe('ProductDetail', () => {
  ...
  test('REQ-F02 edita lote con fecha de vencimiento existente', async () => {
    ...
    expect(onEditBatch).toHaveBeenCalledWith('batch-1', expect.objectContaining({
...
describe('BatchDetail', () => {
  test('REQ-F03 muestra movimiento neto positivo y vencimiento vigente', () => {
    render(
      <BatchDetail
        product={product}
        batch={{ ...baseBatch, expiryDate: new Date('2999-12-31T00:00:00.000Z') }}
  ...
  test('REQ-F03 muestra movimiento neto negativo y vencimiento vencido', () => {
    render(
      <BatchDetail
        product={product}
        batch={{ ...baseBatch, expiryDate: new Date('2020-01-01T00:00:00.000Z') }}
