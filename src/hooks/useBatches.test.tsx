...
import { baseBatchFormData } from "../test/fixtures/inventory";
...
describe("useBatches", () => {
  ...
  test("REQ-F02 carga lotes desde localStorage convirtiendo fechas", () => {
    ...
    expect(result.current.getBatch("batch-2")?.batchNumber).toBe(
      "SIN-VENCIMIENTO",
    );
  });
  ...
  test("REQ-F02 registra, actualiza, consulta y elimina lotes", () => {
    ...
    expect(result.current.getBatch(created.id)?.batchNumber).toBe(
      "LOTE-EDITADO",
    );
  ...
  test("REQ-F02 permite crear un lote sin producto asociado cuando se invoca el hook directamente", () => {
    const { result } = renderHook(() => useBatches());

    act(() => {
      result.current.addBatch("", {
        ...baseBatchFormData,
        batchNumber: "LOTE-SIN-PRODUCTO",
      });
    });

    expect(result.current.batches).toHaveLength(1);
    expect(result.current.batches[0].productId).toBe("");
    expect(result.current.batches[0].batchNumber).toBe("LOTE-SIN-PRODUCTO");
  });
});
