...
describe("useStockMovements", () => {
  ...
  test("REQ-F03 devuelve 0 si el lote no existe", () => {
    ...
    expect(result.current.calculateAvailableQuantity("batch-inexistente")).toBe(
      0,
    );
  });
  ...
  test("REQ-F03 agrega y elimina movimientos aunque el lote no exista", () => {
    ...
    expect(result.current.getMovementsByBatch("batch-faltante")).toHaveLength(
      1,
    );
    expect(updateAvailableQuantityMock).not.toHaveBeenCalled();

    act(() => {
      result.current.deleteMovement(
        result.current.movements[0].id,
        "batch-faltante",
      );
    });
