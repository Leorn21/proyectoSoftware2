...
const fillProductForm = async (
  ...
  await user.selectOptions(
    screen.getByLabelText(/Unidad de Medida/i),
    product.unit,
  );
};
...
const openProductLots = async (user: TestUser, productName = /Martillo/i) => {
  await user.click(
    within(getProductRow(productName)).getByRole("button", { name: /Lotes/i }),
  );
...
const fillBatchForm = async (
  ...
  await user.type(
    screen.getByLabelText(/Cantidad Inicial/i),
...
const openBatchMovements = async (
  user: TestUser,
  batchNumber = /LOTE-001/i,
) => {
  await user.click(
    within(screen.getByRole("row", { name: batchNumber })).getByRole("button", {
      name: /Movimientos/i,
    }),
  );
  await screen.findByRole("heading", {
    name: new RegExp(`Lote ${batchNumber.source ?? batchNumber}`, "i"),
  });
};
...
describe("Gestor de Inventario - trazabilidad funcional", () => {
  ...
  test("REQ-F01 impide crear producto con campos obligatorios vacíos", async () => {
    ...
    expect(
      screen.getByText(/La descripción es requerida/i),
    ).toBeInTheDocument();
    expect(screen.getByText(/La categoría es requerida/i)).toBeInTheDocument();
    expect(
      screen.getByText(/La unidad de medida es requerida/i),
    ).toBeInTheDocument();
  ...
  test("REQ-F01 editar producto", async () => {
    ...
    await user.click(
      within(getProductRow(/Martillo/i)).getByRole("button", {
        name: /Editar/i,
      }),
    );
    await user.clear(screen.getByLabelText(/Nombre del Producto/i));
  ...
  test("REQ-F01 consulta el listado de productos mostrando múltiples registros", async () => {
    const user = await renderInventory();
    await createProduct(user);
    await createProduct(user, {
      code: "MAT-200",
    ...
    expect(
      screen.getByRole("heading", { name: /Lista de Productos/i }),
    ).toBeInTheDocument();
    expect(screen.getByText("Martillo")).toBeInTheDocument();
    expect(screen.getByText("Clavos")).toBeInTheDocument();
  ...
  test("REQ-F01 eliminar producto con confirmación", async () => {
    ...
    expect(
      within(dialog).getByText(/Eliminar producto "Martillo"/i),
    ).toBeInTheDocument();
    await user.click(
  ...
  test("REQ-F02 impide lote sin número y con cantidad inicial menor o igual a 0", async () => {
    ...
    expect(
      screen.getByText(/El número de lote es requerido/i),
    ).toBeInTheDocument();
    expect(
  ...
  test("REQ-F02 editar lote", async () => {
    ...
    await user.click(
      within(screen.getByRole("row", { name: /LOTE-001/i })).getByRole(
        "button",
        { name: /Editar/i },
      ),
    );
  ...
  test("REQ-F02 rechaza fecha de vencimiento anterior a la fecha de ingreso", async () => {
    const user = await renderInventory();
    await createProduct(user);
    await openProductLots(user);

    await user.click(screen.getByRole("button", { name: /\+ Nuevo Lote/i }));
    await fillBatchForm(user, {
    ...
    expect(
      screen.getByText(
        /La fecha de vencimiento no puede ser anterior a la fecha de ingreso/i,
      ),
    ).toBeInTheDocument();
    expect(screen.queryByText("LOTE-VENCIDO")).not.toBeInTheDocument();
  });
  ...
  test("REQ-F04 bloquea egreso mayor al stock disponible y no registra movimiento inválido", async () => {
    ...
    expect(
      screen.getByText(/No hay suficiente stock. Disponible: 10/i),
    ).toBeInTheDocument();
  ...
  test("REQ-F04 verifica que el stock nunca quede negativo", async () => {
    ...
    await registerMovement(user, {
      type: "egreso",
      quantity: "1",
      reason: "Salida negativa",
    });
    expect(
      screen.getByText(/No hay suficiente stock. Disponible: 0/i),
    ).toBeInTheDocument();
  ...
  test("REQ-F05 mantiene la trazabilidad entre producto, lote y movimiento", async () => {
    const user = await renderInventory();
    await createProduct(user, {
      code: "TRAZA-001",
      name: "Taladro",
    ...
    await openProductLots(user, /Taladro/i);
    await createBatch(user, {
      batchNumber: "TRAZA-LOTE-1",
      initialQuantity: "8",
      expiryDate: "",
    });
    await openBatchMovements(user, /TRAZA-LOTE-1/i);
    await registerMovement(user, {
      type: "ingreso",
      quantity: "2",
    ...
    expect(screen.getByText(/Lote TRAZA-LOTE-1/i)).toBeInTheDocument();
    expect(screen.getByText("Taladro")).toBeInTheDocument();
    expect(screen.getByText("Ajuste inicial")).toBeInTheDocument();
    expect(screen.getAllByText("10 piezas").length).toBeGreaterThan(0);
  });

  test("REQ-F03 mantiene el cálculo correcto luego de varios movimientos consecutivos", async () => {
    const user = await renderInventory();
    await createProduct(user);
    await openProductLots(user);
    await createBatch(user);
    ...
    expect(screen.getAllByText("10 piezas").length).toBeGreaterThan(0);
    expect(screen.getByText("Reposición 1")).toBeInTheDocument();
    expect(screen.getByText("Venta 1")).toBeInTheDocument();
    expect(screen.getByText("Reposición 2")).toBeInTheDocument();
  ...
  test("REQ-F05 mantiene los datos después de recargar desde localStorage", async () => {
    ...
    expect(
      screen.getByText(
        (_content, element) => element?.textContent === "1 productos | 1 lotes",
      ),
    ).toBeInTheDocument();
